...
# simulate fix: assign Command.set instead of browse recordset
...
try:
    draft.unavailable_handover_employee_ids = [Command.set(unavailable_ids)]
    print("Command.set assign OK, ids:", draft.unavailable_handover_employee_ids.ids)
except Exception as ex:
    print("Command.set assign FAIL:", type(ex).__name__, ex)

try:
    print("mapped name:", draft.unavailable_handover_employee_ids.mapped("name"))
except Exception as ex:
    print("mapped name FAIL:", type(ex).__name__, ex)
